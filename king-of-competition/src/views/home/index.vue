    ...
    export default {
        ...
        methods: {
            ...
            doUserAuthOrLogin (fn) {
                ...
                let { code, openid, SAUserActiveId } = this.$route.query;
                if (!SAUserActiveId) {
                    // 没有活动 id
                ...
                this.$api.doUserLogin({
                    ...
                    this.$user.update(Object.assign(res, { token, openid, SAUserActiveId }));
            ...
            handleStart (flag) {
                ...
                this.$wx.getLocation({
                    ...
                    const { ActiveStep } = ActiveUser;
                    // 首页
                    // 活动完成阶段：0扫码，1观看视屏，2上传工单，3调研，4审核不通过，5审核通过，6抽奖，7留资/寄送
                    if (ActiveStep === 0) {
                        return this.isVideoPopup = true;
                        ...
                        return this.$vux.toast.show('您的活动状态异常，请联系客服');
                    }
                }).toast();
            },
            handleVideoClose () {
